...
void ImageParser::calculateBresenham(int startX, int startY, int endX, int endY) {
	cv::Point start(startX, startY);
	cv::Point end(endX, endY);
	cv::LineIterator it(m_image, start, end, 8);

	for (int i = 0; i < it.count; i++, ++it) {
		UserPoint userPoint;
		userPoint.value = (int)(*it)[0];
		userPoint.xCoord = (int)(it.pos().x);
		userPoint.yCoord = (int)(it.pos().y);		
		bresenhamPath.insert(bresenhamPath.begin(), userPoint);
